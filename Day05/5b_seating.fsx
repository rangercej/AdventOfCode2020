...
let seatIds = seatLoc |> List.map(fun x -> (fst(x) * 8) + snd(x)) |> List.sort |> set

let seatMax = seatIds |> Set.maxElement
let seatMin = seatIds |> Set.minElement
let allSeats = [ seatMin .. seatMax ] |> set

// F# sets - we take one set away from the other to find the non-matching elements
let freeSeats = allSeats - seatIds

printfn "Free seat = %A" freeSeats
